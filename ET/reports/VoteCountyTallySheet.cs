...
namespace KnightRider.ElectionTracker.reports {
    internal class VoteCountyTallySheet : BaseReport<Election> {
        ...
        private static readonly IComparer<Response> CANDIDATE_BY_NAME = new ResponseComparer(true, true);
        ...
        protected override bool performGenerate(Election entity) {
            ...
            foreach (County county in counties) {
                ...
                foreach (ContestCounty contestCounty in contestCounties) {
                    ...
                    //We need the tally sheet to be sorted by name
                    List<Response> candidate_response = new List<Response>(electionContest.Responses);
                    candidate_response.Sort(CANDIDATE_BY_NAME);
              
                    foreach (Response response in candidate_response)
                    {                        
