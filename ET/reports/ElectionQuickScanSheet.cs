...
namespace KnightRider.ElectionTracker.reports {
    public class ElectionQuickScanSheet : BaseReport<Election> {
        ...
        private const int RESPONSE_COLUMN_WIDTH = 25;
        ...
        private const int CONTEST_COLUMN_WIDTH_HEADER = 40;
        private const int RESPONSE_COLUMN_WIDTH_HEADER = 14;
        private const int REPORTING_COLUMN_WIDTH_HEADER = 19;
        ...
        protected override bool performGenerate(Election entity) {
            ...
            foreach (ElectionContest electionContest in electionContests) {
              ...
              //  body.Add("");
                string contestColumn = PadString(electionContest.Contest.Name, CONTEST_COLUMN_WIDTH_HEADER, true);
                string reportingColumn = PadString((electionContest.GetWardsReportingPercentage() * 100).ToString("0.0") + "%", REPORTING_COLUMN_WIDTH_HEADER, true);
                ...
                foreach (Response response in responses) {
                    ...
                    if (first_pass)
                    {
                        body.Add(contestColumn + PadString(" ", RESPONSE_COLUMN_WIDTH_HEADER) + reportingColumn);
                    }
                   
                    body.Add(PadString(" ", CONTEST_COLUMN_WIDTH) + responseColumn + PadString(" ", REPORTING_COLUMN_WIDTH) + voteColumn);
